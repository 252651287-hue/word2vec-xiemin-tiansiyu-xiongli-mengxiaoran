...
class TransformerTranslator(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=256, n_heads=4, n_layers=3, d_ff=1024, dropout=0.15, max_len=256):
        super().__init__()
...
@torch.no_grad()
def show_examples(model, test_loader, tgt_vocab, device, num_examples=8):
    for src, tgt, raw_src, raw_tgt in test_loader:
        ...
        for s, t, p_ids in zip(raw_src, raw_tgt, pred):
            ...
            for idx in p_ids:
                ...
                if tok not in ["<pad>", "<sos>"]:
                    hyp_toks.append(tok)
            outputs.append((s, t, " ".join(hyp_toks)))
...
def main():
    parser = argparse.ArgumentParser(description="Task1: 用原始 Transformer(点积注意力)实现英法翻译")
    ...
    parser.add_argument("--save_path", type=str, default="task1_best_dot_model.pt")
    ...
    print("=" * 60)
    print("Task 1: 用 Transformer（点积注意力）实现英法翻译任务")
    print(f"设备: {device}")
    print("=" * 60)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val:
            ...
            if bad_epochs >= args.patience:
                print(f"早停触发：连续 {args.patience} 轮验证集未提升。")
                break

    model.load_state_dict(torch.load(args.save_path, map_location=device))
    bleu1, bleu4 = evaluate_bleu(model, test_loader, tgt_vocab, device, decode_max_len=args.max_len + 10)
    print("\n===== Task 1 测试集结果 =====")
    print(f"测试集 BLEU-1: {bleu1:.2f}")
    print(f"测试集 BLEU-4: {bleu4:.2f}")

    print("\n===== Task 1 翻译示例 =====")
    examples = show_examples(model, test_loader, tgt_vocab, device, num_examples=8)
    ...
    print(f"最优模型已保存: {args.save_path}")
